<template>
  ...
  const { isDarkTheme, toggleAppTheme } = useAppTheme();
  ...
  watch(
    ...
    (uid) => {
      ...
      notifications.value = [];
      if (!uid) { unsubscribeNotifications = null; return; }
      unsubscribeNotifications = watchNotifications(
        uid,
        (items) => { notifications.value = items; },
        (error) => { console.error("Error loading notifications:", error); notifications.value = []; }
      );
    },
    { immediate: true }
  );

  onBeforeUnmount(() => { unsubscribeNotifications?.(); });

  function switchMode(mode) {
    if (!mode || mode === appContext.activeMode) return;
    appContext.setActiveMode(mode);
    router.push(defaultRouteForMode(mode));
  }

  function defaultRouteForMode(mode) {
    if (mode === "psychologist") return "/psicologo/sesiones";
    if (mode === "admin") return "/pacientes";
    return "/dashboard";
  }

  async function openNotification(notification) {
    if (!notification?.id || openingNotificationId.value) return;
    openingNotificationId.value = notification.id;
    try {
      if (!notification.readAt) await markNotificationAsRead(notification.id);
      if (notification.route) await router.push(notification.route);
    } catch (error) {
      console.error("Error marking notification as read:", error);
      window.dispatchEvent(new CustomEvent("api-error", { detail: { message: "No se pudo abrir la notificación." } }));
    } finally {
  ...
  :global(.v-theme--light) .app-top-nav::after { background: transparent; }
  ...
  .mode-switch { max-width: min(44vw, 420px); overflow: hidden; }
  .mode-chip { margin-inline-end: 4px; }
  ...
  .notifications-menu { max-height: min(70vh, 520px); overflow-y: auto; }
  .notification-unread { background: rgba(var(--v-theme-secondary), 0.08); }

  :global(.v-theme--light) .nav-title { color: rgb(var(--v-theme-on-background)); font-weight: 600; }
  ...
  .nav-icon-btn :deep(.v-btn__underlay) { opacity: 0 !important; }

  @media (max-width: 600px) {
    .app-top-nav { min-height: 64px; }
    .app-top-nav__content { gap: 4px; height: 64px; padding-inline: 12px; }
    .nav-title { font-size: 1.1rem !important; }
    :deep(.v-btn--icon) { height: 44px; width: 44px; }
    .nav-actions { gap: 2px; }
    .nav-icon-btn { flex-basis: 40px; }
  }
  </style>
